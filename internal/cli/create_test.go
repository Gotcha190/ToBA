...
func TestRunCreateCreatesProjectSkeletonFromSSHStarter(t *testing.T) {
	...
	assertHasCommand(t, runner.commands, "ssh", []string{"-p", "22666", "toba@185.238.75.243", "dynamic:home"})
	assertHasCommand(t, runner.commands, "scp", []string{"-P", "22666", "dynamic:remote-sql", "dynamic:local-sql"})
...
func TestRunCreateFailsWhenExistingProjectFolderContainsProjectMarkers(t *testing.T) {
	...
	err := runCreateWithRunner(CreateOptions{Name: "demo", SSHTarget: "toba@185.238.75.243 -p 22666"}, &fakeRunner{})
	if err == nil {
